...
def union_decode(img, tile_px, max_windows=72):
    """Three-pronged: bbox-detect whole image, grid-aligned windows, fine sliding windows."""
    tw, th = img.size
    win = max(224, int(tile_px))
    attempts = 0
    # prong 1: DETECTFIRST on whole image (works for crops with intact watermark region)
    if tw >= 224 and th >= 224:
        attempts += 1
        if dec_ok(tm_det, img, detectfirst=True):
            return True, attempts, ("detect", None)
    # prong 2: windows aligned to image-origin tile grid (resize/reproject keep phase)
    cands = []
    for gy in range(0, th - win + 1, win):
        for gx in range(0, tw - win + 1, win):
            cands.append((gx, gy, 0))
    # prong 3: fine sliding for arbitrary crop phase
    step = max(96, win // 8)
...
(OUT / "report_scene_b1_v3.json").write_text(json.dumps({"expected": hex(EXPECTED), "results": rows}, ensure_ascii=False, indent=2), encoding="utf-8")
