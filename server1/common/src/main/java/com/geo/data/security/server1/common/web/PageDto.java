package com.geo.data.security.server1.common.web;
...
public record PageDto<T>(
    ...
    public static <T> PageDto<T> of(List<T> items, long total, int page, int pageSize) {
        return new PageDto<>(items, total, page, pageSize, totalPages(total, pageSize));
    }

    public static <T> PageDto<T> slice(List<T> all, Integer page, Integer pageSize) {
        ...
        List<T> items = from >= all.size()
                ? List.of()
                : List.copyOf(all.subList(from, Math.min(from + size, all.size())));
        return new PageDto<>(items, total, current, size, pages);
    }
}
