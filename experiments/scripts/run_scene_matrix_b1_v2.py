...
def scan_decode(img, tile_px, max_windows=64):
    """Two-phase: (1) grid-aligned windows (scale/pad attacks keep phase); (2) fine sliding (crops shift phase)."""
    tw, th = img.size
    win = max(224, int(tile_px))
    # phase 1: aligned to image-origin grid (resize/reproject back to orig grid keep phase 0)
    cands = []
    for gy in range(0, th - win + 1, win):
        for gx in range(0, tw - win + 1, win):
            cands.append((gx, gy))
    # phase 2: sliding every ~128px to cover any crop phase
    step = max(96, win // 8)
...
# random crops 25% / 75%, 3 batches each
...
# reproject: 32644 -> 4326 -> back to ORIGINAL grid, scan
...
(OUT / "report_scene_b1_v2.json").write_text(json.dumps({"expected": hex(EXPECTED), "results": rows}, ensure_ascii=False, indent=2), encoding="utf-8")
